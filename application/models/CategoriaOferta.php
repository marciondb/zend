...
class Application_Model_CategoriaOferta extends Application_Model_Abstract
{
    public function __construct() {
        $this->_dbTable = new Application_Model_DbTable_CategoriaOferta();
    }
    ...
    public function gravar($parametros, $update = FALSE,$where = false)
    {
        if(!$update)
        {
            try
            {                
                $id_categoria_oferta = $this->save($parametros);    
                
                return (int)$id_categoria_oferta;
            }
            catch(Exception $e)
            {
                ZendUtils::transmissorMsg('Erro ao cadastrar a Oferta, tente novamente mais tarde. Caso o erro persista, entre em contato com a o administrador!<br>'.$e->getMessage(),  ZendUtils::MENSAGEM_ERRO,  ZendUtils::MENSAGEM_SEM_TEMPO);
            }
        } else
        {
            try
            ...
            {
                ZendUtils::transmissorMsg('Erro ao atualizar a Oferta. Tente novamente mais tarde. Caso o erro persista, entre em contato com a o administrador!<br>'.$e->getMessage(),  ZendUtils::MENSAGEM_ERRO,  ZendUtils::MENSAGEM_SEM_TEMPO);
                return $e->getMessage();
            }
        }
    ...
    public function deletar($id_categoria_oferta)
    {
        try 
        {       
            $this->delete(array('id_categoria_oferta=?'=>$id_categoria_oferta));
        }
        catch(Exception $e)
        {
            ZendUtils::transmissorMsg('Erro ao apagar a Oferta, favor contactar o administrador<br>'.$e->getMessage(),  ZendUtils::MENSAGEM_ERRO,  ZendUtils::MENSAGEM_SEM_TEMPO);
        }
    }
    ...
    {                  
        ...
        try{
            $select = $this->_dbTable->
                    ...
                    from('categoria_oferta',array('id_categoria_oferta','titulo'));
            
            $select->order('categoria_oferta.titulo');
